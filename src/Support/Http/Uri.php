...
class Uri implements UriInterface
{
    ...
    /**
     * Get a Uri object from a URI string that may contain unencoded characters
     */
    public static function from(string $uri): self
    {
        // Percent-encode everything except reserved, unreserved and pre-encoded
        // characters
        return new self(Pcre::replaceCallback(
            '/(%(?![0-9a-f]{2})|[^]!#$%&\'()*+,\/:;=?@[])+/i',
            fn(array $matches) =>
    ...
    private function validate(): self
    {
        ...
        if ($this->getAuthority() === '') {
            ...
            if ($this->Scheme === '' && $this->Path !== '' && Pcre::match('/^[^\/]*:/', $this->Path)) {

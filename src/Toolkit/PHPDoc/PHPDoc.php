...
final class PHPDoc implements Immutable
{
    ...
    private function parse(string $content): void
    {
        ...
        foreach ($this->TagsByName as $key => $tags) {
            $this->TagsByName[$key] = Arr::whereNotEmpty($tags);
        }
    }

    /**
     * Get an instance where the PHPDoc's "@var" tags are normalised
     *
     ...
     * @return static
     */
    public function normaliseVars()
    {
        if (
            count($this->Vars) !== 1
            || array_key_first($this->Vars) !== 0
        ) {
            return $this;
        }
        ...
        $var = $var->withDescription(null);
        $normalised = $this->with('Vars', [$var]);
        if ($this->Summary === null) {
            return $normalised->with('Summary', $description);
        } elseif ($this->Summary !== $description) {
            return $normalised->with('Description', Arr::implode("\n\n", [
                $this->Description,
                $description,
            ], ''));
        }
    ...
    /**
     * Collapse one or more consecutive whitespace characters in a string to a
     * single space, preserving null
     */
    public static function unwrap(?string $string): ?string
    {
        return $string === null
            ? null
    ...
    /**
     * Get the PHPDoc's original tags, in order of appearance
     *
     * @return string[]
     */
    ...
    /**
     * Get the PHPDoc's original tag metadata, indexed by tag name
     *
     * @return array<string,string[]>
     */
    public function getTagsByName(): array
    {

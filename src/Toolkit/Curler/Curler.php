...
class Curler implements CurlerInterface, Buildable
{
    ...
    public function getHttpHeaders(): HttpHeadersInterface
    {
        ...
        if ($this->ExpectJson) {
            $headers = $headers->set(HttpHeader::ACCEPT, self::TYPE_JSON);
        }
        if ($this->UserAgent !== '' && (
            $this->UserAgent !== null
    ...
    public function withRequest(RequestInterface $request)
    {
        ...
        $expectJson = Arr::lower($headers->getHeaderValues(HttpHeader::ACCEPT)) === [self::TYPE_JSON];
    ...
    private function applyData(HttpRequest $request, $data): HttpRequest
    {
        if ($data === null) {
            ...
            return $request->withHeader(HttpHeader::CONTENT_TYPE, self::TYPE_FORM);
        }
        if (is_string($data)) {

 ...
 * @method static ConsoleInterface count(Level::* $level) Record a message with level $level without printing anything
 ...
 * @method static ConsoleInterface deregisterTarget(ConsoleTargetInterface $target) Deregister and close a registered target (see {@see ConsoleInterface::deregisterTarget()})
 ...
 * @method static ConsoleInterface exception(Throwable $exception, Level::* $level = Level::ERROR, Level::*|null $traceLevel = Level::DEBUG) Print an exception's name and message with a given level, optionally followed by its stack trace with a different level (see {@see ConsoleInterface::exception()})
 * @method static int getErrorCount() Get the number of error messages recorded by the writer so far
 * @method static FormatterInterface getFormatter(Level::* $level = Level::INFO) Get an output formatter for a registered target (see {@see ConsoleInterface::getFormatter()})
 ...
 * @method static int|null getWidth(Level::* $level = Level::INFO) Get the width of a registered target in columns (see {@see ConsoleInterface::getWidth()})
 ...
 * @method static ConsoleInterface message(string $msg1, string|null $msg2 = null, Level::* $level = Level::INFO, MessageType::* $type = MessageType::UNDECORATED, Throwable|null $ex = null, bool $count = true) Print "$msg1 $msg2" with prefix and formatting optionally based on $level
 * @method static ConsoleInterface messageOnce(string $msg1, string|null $msg2 = null, Level::* $level = Level::INFO, MessageType::* $type = MessageType::UNDECORATED, Throwable|null $ex = null, bool $count = true) Print "$msg1 $msg2" with prefix and formatting optionally based on $level once per run
 * @method static ConsoleInterface print(string $msg, Level::* $level = Level::INFO, MessageType::* $type = MessageType::UNFORMATTED) Print "$msg" to registered targets
 * @method static ConsoleInterface printOut(string $msg, Level::* $level = Level::INFO, MessageType::* $type = MessageType::UNFORMATTED) Print "$msg" to registered STDOUT or STDERR targets
 * @method static ConsoleInterface printStderr(string $msg, Level::* $level = Level::INFO, MessageType::* $type = MessageType::UNFORMATTED) Print "$msg" to STDERR even if no STDERR target is registered
 * @method static ConsoleInterface printStdout(string $msg, Level::* $level = Level::INFO, MessageType::* $type = MessageType::UNFORMATTED) Print "$msg" to STDOUT even if no STDOUT target is registered
 * @method static ConsoleInterface printTty(string $msg, Level::* $level = Level::INFO, MessageType::* $type = MessageType::UNFORMATTED) Print "$msg" to registered TTY targets
 ...
 * @method static ConsoleInterface registerTarget(ConsoleTargetInterface $target, array<Level::*> $levels = LevelGroup::ALL) Register a target to receive console output
 * @method static ConsoleInterface setTargetPrefix(string|null $prefix, int-mask-of<ConsoleTargetTypeFlag::*> $flags = 0) Set or unset the prefix applied to each line of output by any registered targets that implement ConsoleTargetPrefixInterface

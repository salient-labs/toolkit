 ...
 * @method static ConsoleWriterInterface count(MessageLevel::* $level) Record a message with level $level without printing anything
 ...
 * @method static ConsoleWriterInterface exception(Throwable $exception, MessageLevel::* $level = MessageLevel::ERROR, MessageLevel::*|null $traceLevel = MessageLevel::DEBUG) Print an exception with level $level (default: ERROR) and its stack trace with level $traceLevel (default: DEBUG) (see {@see ConsoleWriterInterface::exception()})
 * @method static int getErrorCount() Get the number of error messages recorded by the writer so far
 * @method static ConsoleFormatter getFormatter(MessageLevel::* $level = MessageLevel::INFO) Get an output formatter for a registered target (see {@see ConsoleWriterInterface::getFormatter()})
 ...
 * @method static ConsoleTargetInterface[] getTargets(MessageLevel::*|null $level = null, int-mask-of<ConsoleTargetTypeFlag::*> $flags = 0) Get a list of registered targets, optionally filtered by level and type
 * @method static int getWarningCount() Get the number of warning messages recorded by the writer so far
 * @method static int|null getWidth(MessageLevel::* $level = MessageLevel::INFO) Get the width of a registered target in columns (see {@see ConsoleWriterInterface::getWidth()})
 ...
 * @method static ConsoleWriterInterface message(MessageLevel::* $level, string $msg1, string|null $msg2 = null, ConsoleMessageType::* $type = ConsoleMessageType::STANDARD, Throwable|null $ex = null, bool $count = true) Print "$msg1 $msg2" with prefix and formatting optionally based on $level
 * @method static ConsoleWriterInterface messageOnce(MessageLevel::* $level, string $msg1, string|null $msg2 = null, ConsoleMessageType::* $type = ConsoleMessageType::STANDARD, Throwable|null $ex = null, bool $count = true) Print "$msg1 $msg2" with prefix and formatting optionally based on $level once per run
 * @method static ConsoleWriterInterface print(string $msg, MessageLevel::* $level = MessageLevel::INFO, ConsoleMessageType::* $type = ConsoleMessageType::UNFORMATTED) Print "$msg" to registered targets
 * @method static ConsoleWriterInterface printOut(string $msg, MessageLevel::* $level = MessageLevel::INFO, ConsoleMessageType::* $type = ConsoleMessageType::UNFORMATTED) Print "$msg" to registered STDOUT or STDERR targets
 * @method static ConsoleWriterInterface printStderr(string $msg, MessageLevel::* $level = MessageLevel::INFO, ConsoleMessageType::* $type = ConsoleMessageType::UNFORMATTED) Print "$msg" to STDERR even if no STDERR target is registered
 * @method static ConsoleWriterInterface printStdout(string $msg, MessageLevel::* $level = MessageLevel::INFO, ConsoleMessageType::* $type = ConsoleMessageType::UNFORMATTED) Print "$msg" to STDOUT even if no STDOUT target is registered
 * @method static ConsoleWriterInterface printTty(string $msg, MessageLevel::* $level = MessageLevel::INFO, ConsoleMessageType::* $type = ConsoleMessageType::UNFORMATTED) Print "$msg" to registered TTY targets
 ...
 * @method static ConsoleWriterInterface registerTarget(ConsoleTargetInterface $target, array<MessageLevel::*> $levels = MessageLevelGroup::ALL) Register a target to receive console output
